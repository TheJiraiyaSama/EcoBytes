...
class PlantInfoRepo {
  ...
  TaskEither<AppException, int> getPlantId({required String scientificName}) {
    return TaskEither(() async {
      final data =
          await _trefleAPIDS.getPlantId(scientificName: scientificName).run();

      return data;
    });
  }

  TaskEither<AppException, PlantDetail> getPlantDetail({required int id}) {
    return TaskEither(() async {
      final data = await _trefleAPIDS.getPlantDetails(id: id).run();
